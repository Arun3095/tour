...
if(isset($_POST['pid']) and $_POST['pid']!='')
{
	...
	if($_POST['pid']==1)
	{
		...
		<a href="<?=PATH?>book-now.php/?id=<?=$fetch1['packageID']?>" class="cus-btn">Book Online</a>
		<a href="<?=PATH?>book-now.php/?id=<?=$fetch1['packageID']?>" class="cus-btn">Contact Us</a>
		<?php 
...
else if(isset($_POST['pid_book']) and $_POST['pid_book']!='')
{
   $query1 = "Select pricing.* FROM pricing where pricing.active='1' and pricing.packageID='".$_POST['packageid_book']."' order by pricing.ID ASC";
	...
	if($_POST['pid_book']==1)
	{
		...
		<span class="ramount1">₹ <?=$fetch1['std_final_price'];?></span><span><strike>₹ <?=$fetch1['std_regular_price']?></strike></span>
		...
		$lux = $_POST['pid_book'];
		?>
		<span class="ramount1">₹ <?=$fetch1['lux_final_price']?></span><span><strike>₹ <?=$fetch1['lux_regular_price']?></strike></span>
		...
		<span class="ramount1">₹ <?=$fetch1['prm_final_price']?></span><span><strike>₹ <?=$fetch1['prm_regular_price']?></strike></span>
                                 ...
                                 <div class="booking-box">
                                    ...
                                    <div class="select-d-box">
                                       <select id="from_city" name="from_city" onchange="Get_City_price(this.value)">
                                          ...
                                          <option value="0">Join Directly</option>
                        ...
                        <div class="paymenyopt1">
                           <center>
                              <h3 class="" id="packprice">
                                 ...
                                 <span id="package_final_price" class="ramount1">₹<?=$_SESSION['final_cost']=$packages[0]["std_final_price"]?></span>
                                 <br>
...
/*----------------------------GET base std & luxury & oremium price---------------------------*/
   var get_price = 0;
   var city_price = 0;
   var room_price = 0;
