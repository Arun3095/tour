...
if(isset($_REQUEST["submit"]))
{
	...
	if($package!= "")
	{
		...
		if($checkinsup==1)
		{
			$qry="update pricing set categoryID='".$category."',subcategoryID='".$subcategory."',packageID='".$package."',planID='".$planID."',lux_final_price='".$lux_final_price."',lux_regular_price='".$lux_regular_price."',std_final_price='".$std_final_price."',std_regular_price='".$std_regular_price."',prm_final_price='".$prm_final_price."',prm_regular_price='".$prm_regular_price."',narration='".$narration."',active='".$active."',u_date='".$E_Date."' where ID='".$edID."'";
		}
		else if($checkinsup==0)
		{
			$qry="insert into pricing set categoryID='".$category."',subcategoryID='".$subcategory."',packageID='".$package."',planID='".$planID."',lux_final_price='".$lux_final_price."',lux_regular_price='".$lux_regular_price."',std_final_price='".$std_final_price."',std_regular_price='".$std_regular_price."',prm_final_price='".$prm_final_price."',prm_regular_price='".$prm_regular_price."',narration='".$narration."',active='".$active."',e_date='".$E_Date."'";
		}
		$res=$db->TransQry($qry,$con);

		if($_POST['price_from_to'][0] > 0 )
		{  
			$dquery =  "delete from member_add_on_price where package_id='".$edID."' and ID NOT IN (".implode(',',$_POST['price_from_to']).") "; 
			$pres=$db->TransQry($dquery,$con);
		}
		...
		for($i=0;$i<=count($_POST['price_from_to']);$i++)
		{
			...
			if($_POST['price_from_to'][$i]!='' and $_POST['price_from_to'][$i]>=0)
			{
				if($_POST['price_from_to'][$i]!=0)
				{
					...
					echo $query ="insert into add_on_price SET package_id='".$get_lastID."', category_id = '".$_REQUEST["category"]."',pricing_id='".$get_lastID_data['ID']."' ,subcategory_id = '".$_REQUEST["subcategory"]."',city = '".$_POST['to_city_price'][$i]."',price = '".$_POST['price_from_to'][$i]."', final_price = '".$_POST['final_price'][$i]."' ";
				}
				else
				{
					echo $query ="update add_on_price SET pricing_id='".$edID."' ,category_id = '".$_REQUEST["category"]."', subcategory_id = '".$_REQUEST["subcategory"]."', package_id = '".$get_lastID."',city = '".$_POST['to_city_price']."',price = '".$_POST['price_from_to']."', final_price = '".$_POST['final_price'][$i]."' where pricing_id ='".$get_lastID_data['ID']."'";
				}
...
{		  
						...
						<!-- Fetched data of add_on_table -->
								...
								while($seriesdetail =mysqli_fetch_assoc($run_psqry))
								{
									...
									<div class="form-group row">
										...
										<input type="hidden" name="pricing_all_id[]" value="<?=$seriesdetail['ID']?>">
										<div class="col-sm-4">
											<select id="to_city_price" class="form-control" name="to_city_price[]" required>
												...
												while($row2 = mysqli_fetch_assoc($run))
												{
													...
													{
														?>
														<option value="<?=$row3['cityID']?>" <?=($seriesdetail['city'] ==$row3['cityID'])?'selected':''?>><?=ucwords($row3['city'])?></option>
													<?php  }
												}
												?>
											</select>
						...
						for($i=0;$i<count($all_data);$i++)
						{
							...
							<tr>
								...
								<td><a href="?route=<?=$PageRoute?>&ed=<?=$all_data[$i]["ID"];?>" class="btn btn-success">Edit</a>
									<a href="?route=<?=$PageRoute?>&del=<?=$all_data[$i]["ID"];?>" class="btn btn-danger">Delete</a></td>
	...
	// getCity();
		...
		$('#add_price_member_based').click(function()
		{
			...
			$("#more_field_member_based_price").append('<div id="remove_field_member_price'+p_srNo+'"><div class="form-group row"><label for="example-text-input" class="col-sm-2 col-form-label">City</label><div class="col-sm-4"><select class="form-control" name="to_city_price[]" required><option selected disabled>---Select City---</option><?php $query1="select * from mast_state where countryID = 81  order by stateID desc "; $run = mysqli_query($con,$query1);while($row2 = mysqli_fetch_assoc($run)){?> <optgroup label="&nbsp;&nbsp;<?=$row2['state']?>"></optgroup><?php $query2 ="select * from mast_city where countryID =81  and stateID ='".$row2['stateID']."' order by cityID desc "; $run_2 = mysqli_query($con,$query2);while($row3 = mysqli_fetch_assoc($run_2)){ ?><option value="<?=$row3['cityID']?>"><?=$row3['city']?></option><?php }}?></select></div><label for="example-text-input" class="col-sm-2 col-form-label">Price</label><div class="col-md-3"><input type="text" name="price_from_to[]" class="form-control" placeholder="Enter Price"></div><div class="col-md-1"><button type="button" class="fa fa-minus" aria-hidden="true" style="border-radius: 50%;font-size: 29px;color: #eef5ee; background: #f13e5f;border: none;" onclick="remove_member_price('+p_srNo+')"></button></div></div></div>');p_srNo++;
		});
